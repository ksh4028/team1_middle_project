# rfp_metadata에서 실제 도메인-키워드 사용 빈도 통계 함수
def view_rfp_metadata_domain_keyword_stats(top_domain_n=10, top_keyword_n=20):
    """rfp_metadata에서 실제 도메인-키워드 사용 빈도 통계 그래프"""
    db = SQLiteDB()
    # domain, keywords 컬럼 추출
    sql = "SELECT domain, keywords FROM rfp_metadata"
    rows = db.select(sql)
    # 도메인-키워드 빈도 집계
    from collections import Counter, defaultdict
    domain_keyword_counter = Counter()
    domain_counter = Counter()
    keyword_counter = Counter()
    for domain_str, keywords_str in rows:
        # 도메인: 콤마로 분리
        ...
        for d in domains:
            domain_counter[d] += 1
            for k in keywords:
                domain_keyword_counter[(d, k)] += 1
                keyword_counter[k] += 1
    # 상위 도메인/키워드 선정
    top_domains = [d for d, _ in domain_counter.most_common(top_domain_n)]
    top_keywords = [k for k, _ in keyword_counter.most_common(top_keyword_n)]
    # 매트릭스 생성
    import numpy as np
    import matplotlib.pyplot as plt
    matrix = np.zeros((len(top_domains), len(top_keywords)), dtype=int)
    for i, d in enumerate(top_domains):
        for j, k in enumerate(top_keywords):
            matrix[i, j] = domain_keyword_counter[(d, k)]
    plt.figure(figsize=(12, 6))
    plt.imshow(matrix, cmap='Oranges', aspect='auto')
    plt.colorbar(label='빈도')
    plt.xticks(np.arange(len(top_keywords)), top_keywords, rotation=45, ha='right')
    plt.yticks(np.arange(len(top_domains)), top_domains)
    plt.xlabel('키워드')
    plt.ylabel('도메인')
    plt.title('도메인-키워드 사용 빈도 히트맵')
    plt.tight_layout()
    plt.show()


    # 지역별 도메인 분포도 추가로 시각화
    sql_region = "SELECT region, domain FROM rfp_metadata"
    region_rows = db.select(sql_region)
    from collections import Counter, defaultdict
    region_domain_counter = Counter()
    region_counter = Counter()
    domain_counter2 = Counter()
    for region_str, domain_str in region_rows:
        region = str(region_str).strip()
        domains = [d.strip() for d in str(domain_str).split(',') if d.strip()]
        region_counter[region] += 1
        for d in domains:
            region_domain_counter[(region, d)] += 1
            domain_counter2[d] += 1
    # 상위 지역/도메인 선정
    top_regions = [r for r, _ in region_counter.most_common(20)]
    top_domains2 = [d for d, _ in domain_counter2.most_common(10)]
    import numpy as np
    import matplotlib.pyplot as plt
    matrix2 = np.zeros((len(top_regions), len(top_domains2)), dtype=int)
    for i, r in enumerate(top_regions):
        for j, d in enumerate(top_domains2):
            matrix2[i, j] = region_domain_counter[(r, d)]
    plt.figure(figsize=(12, 6))
    plt.imshow(matrix2, cmap='Greens', aspect='auto')
    plt.colorbar(label='빈도')
    plt.xticks(np.arange(len(top_domains2)), top_domains2, rotation=45, ha='right')
    plt.yticks(np.arange(len(top_regions)), top_regions)
    plt.xlabel('도메인')
    plt.ylabel('지역')
    plt.title('지역별 도메인 분포 히트맵')
    plt.tight_layout()
    plt.show()

    # 예산별 도메인 분포도 추가로 시각화
    sql_budget = "SELECT budget, domain FROM rfp_metadata"
    budget_rows = db.select(sql_budget)
    # 1억 미만부터 10억 이상까지 1억 단위로 구간 생성
    budget_bins = [i * 100000000 for i in range(1, 10)]  # 1억, 2억, ..., 9억
    bin_labels = [f'{i}억 미만' for i in range(1, 10)] + ['10억 이상']
    budget_domain_counter = Counter()
    bin_counter = Counter()
    domain_counter3 = Counter()
    for budget, domain_str in budget_rows:
        try:
            b = float(budget)
        except:
            b = 0
        bin_idx = 0
        for i, upper in enumerate(budget_bins):
            if b < upper:
                bin_idx = i
                break
        else:
            bin_idx = len(budget_bins)
        bin_label = bin_labels[bin_idx] if bin_idx < len(bin_labels) else bin_labels[-1]
        domains = [d.strip() for d in str(domain_str).split(',') if d.strip()]
        bin_counter[bin_label] += 1
        for d in domains:
            budget_domain_counter[(bin_label, d)] += 1
            domain_counter3[d] += 1
    top_bins = bin_labels
    top_domains3 = [d for d, _ in domain_counter3.most_common(10)]
    matrix3 = np.zeros((len(top_bins), len(top_domains3)), dtype=int)
    for i, bin_label in enumerate(top_bins):
        for j, d in enumerate(top_domains3):
            matrix3[i, j] = budget_domain_counter[(bin_label, d)]
    plt.figure(figsize=(12, 6))
    plt.imshow(matrix3, cmap='Purples', aspect='auto')
    plt.colorbar(label='빈도')
    plt.xticks(np.arange(len(top_domains3)), top_domains3, rotation=45, ha='right')
    plt.yticks(np.arange(len(top_bins)), top_bins)
    plt.xlabel('도메인')
    plt.ylabel('예산 구간')
    plt.title('예산별 도메인 분포 히트맵')
    plt.tight_layout()
    plt.savefig('budget_domain_heatmap.png')
    plt.pause(2)
    plt.show()
...
# 도메인-키워드 히트맵만 별도 함수로 분리
def view_domain_keyword_heatmap(top_domain_n=10, top_keyword_n=20):
    """도메인-키워드 조합 히트맵만 시각화"""
    db = SQLiteDB()
    sql = "SELECT category, keyword FROM domain_keywords"
    rows = db.select(sql)

    # 도메인별 키워드 집합, 키워드별 도메인 집합
    category_counts = {}
    keyword_counts = {}
    domain_keyword_pairs = []
    for category, keyword in rows:
    ...
    import matplotlib.pyplot as plt
    # 상위 도메인/키워드 선정
    top_domains = sorted(category_counts.keys(), key=lambda c: len(category_counts[c]), reverse=True)[:top_domain_n]
    top_keywords = [k for k, _ in sorted(keyword_counts.items(), key=lambda x: len(x[1]), reverse=True)[:top_keyword_n]]
    # 매트릭스 생성
    matrix = np.zeros((len(top_domains), len(top_keywords)), dtype=int)
    pair_set = set(domain_keyword_pairs)
    for i, d in enumerate(top_domains):
...
if __name__ == "__main__":
    view_rfp_metadata_domain_keyword_stats(100,50)
  #  view_domain_keyword_heatmap()
