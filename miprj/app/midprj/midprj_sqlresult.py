...
class SQLSearchHelper:
    ...
    def sql_metadata_search(filters):
        ...
        if not query_parts:
            ...
            sql = "SELECT * FROM rfp_metadata order by project_name LIMIT 10"
            rows = db.select(sql)
        else:
            where_clause = " AND ".join(query_parts)
            sql = f"SELECT * FROM rfp_metadata WHERE {where_clause} order by project_name LIMIT 15"
        ...
        for row in rows:
            p_name = row[2]
            n_no = row[0]
            # 사업명과 공고번호 조합으로 중복 체크
            if (p_name, n_no) in seen_projects:
                continue
            seen_projects.add((p_name, n_no))
    ...
    def collect_metadata_from_docs(docs) -> list[dict]:
        ...
        for doc in docs or []:
            ...
            notice_no = meta.get("Notice_no") or ""
            notice_round = meta.get("Notice_round") or ""
            project_name = meta.get("project_name") or ""
            file_name = meta.get("file_name") or ""
            # project_name 기준으로 중복 제거 (None 방지)
            key = (notice_no, project_name, file_name)
            if key in seen:
                continue
            seen.add(key)
            items.append({
