...
class QueryFilterExtractor:
    ...
    _results_cache = {}  # 질문별 필터 추출 결과 캐시
    ...
    def extract(self, query: str) -> SearchFilter:
        ...
        if query in self._results_cache:
            # Lines(f"[DEBUG] QueryFilterExtractor - 캐시된 필터 사용: {query}")
            return self._results_cache[query].model_copy()
        ...
        try:
            ...
            # 결과 캐싱
            if len(self._results_cache) > 100: # 메모리 관리 (최근 100개만 유지)
                self._results_cache.clear()
            self._results_cache[query] = filters
...
class BaseLLMModel():
    _SYSTEM_CONSTRAINTS = (
        ...
        "4. 여러 사업을 나열하거나 설명을 전환할 때(예: 관련 문서 요약 후 데이터베이스 조회 결과 설명 등), 각 항목과 항목 사이에 반드시 명확한 구분선('--------------------------------------------------')을 삽입하여 가독성을 높이세요.\n"
    )
    _PROMPT_TEMPLATE = (
        "{basequery}\n\n시스템 제약조건:\n{constraints}\n\n"
        "Context:\n{context}\n\nQuestion: {question}"
    ...
    def rag_search(self, question):
        ...
        if self._ensemble_retriever is None:
            self._ensemble_retriever = build_retriever_from_param(vector_store=self._vector_store, 
                                                                  ...
                                                                  do_retriever=self._param.do_retriever)
    ...
    def _perform_rag_search(self, question, retriever):
        ...
        try:
            ...
            sql_results = ""
            sql_items = []
            if self._param.use_sqlite and SQLSearchHelper.detect_search_intent(question, filters):
                OpLog(f"검색 의도 감지됨 (SQLite 사용: {self._param.use_sqlite}) - SQL 검색 수행", level="INFO")
            ...
            # 4-1. 중복 문서 제거 (page_content 기준)
            _seen_contents = set()
            _deduped_docs = []
            for _doc in all_retrieved_docs:
                _key = _doc.page_content.strip()
                if _key not in _seen_contents:
                    _seen_contents.add(_key)
                    _deduped_docs.append(_doc)
            if len(all_retrieved_docs) != len(_deduped_docs):
                OpLog(f"중복 문서 제거: {len(all_retrieved_docs)}개 → {len(_deduped_docs)}개", level="INFO")
            all_retrieved_docs = _deduped_docs
            ...
            # 7. SQL 결과와 문서 검색 결과 병합 (공고번호 기준 중복 제거)
            sql_items = sql_items or []
            doc_items = doc_items or []
            seen_notice_nos = {str(item.get("Notice_no", "")).strip() for item in sql_items if item.get("Notice_no")}
            unique_doc_items = [
                item for item in doc_items 
                if str(item.get("Notice_no", "")).strip() not in seen_notice_nos
            ]
            metadata_items = sql_items + unique_doc_items
            ...
            # SQLite 사용 여부에 따른 동적 시스템 제약 조건 설정
            dynamic_constraints = (
                "1. 사용자가 요청한 금액(예산)과 기관 조건을 반드시 엄격히 준수하여 답변하세요.\n"
                "2. 여러 사업을 나열하거나 설명을 전환할 때, 각 항목과 항목 사이에 반드시 명확한 구분선('--------------------------------------------------')을 삽입하여 가독성을 높이세요.\n"
            )
            if self._param.use_sqlite:
                dynamic_constraints += (
                    "3. 아래 [데이터베이스 직접 조회 결과]에 항목이 있다면, 이는 검색 엔진이 정확하게 찾아낸 것이므로 **단 하나도 빠뜨리지 말고 모두** 목록에 포함하여 답변하세요.\n"
                    "4. 만약 [데이터베이스 직접 조회 결과]와 [관련 문서 내용]의 정보가 충돌한다면 데이터베이스 직접 조회 결과를 우선하세요.\n"
                )
            ...
            if is_core_query:
                prompt_prefix_base = (
                    ...
                    "0) 핵심 기능: 핵심 기능을 불릿으로 나열\n"
                    "1) 해당 사업명: 아래 목록에서 해당하는 사업명만 불릿으로 나열\n\n"
                    "※ 주의: 사업을 나열할 때, 각 사업 정보 시작 전에 반드시 다음 구분선을 넣어주세요:\n"
                    "--------------------------------------------------\n\n"
...
def Make_Model(model_item:str):
    ...
    if model_item == "openai":
        ...
        param.temperature = 0.0          # 완전한 사실 기반 (0.1보다 더 엄격하게 설정 가능)
        param.repetition_penalty = 1.1   # 답변의 다양성보다 정확성 강조
        param.chunk_size = 400           # 300보다 약간 늘려 의미가 끊기지 않게 조정
        param.chunk_overlap = 40         # 청크 간 연결성 확보
        param.k = 4                      # 상위 4개 문서만으로도 충분히 답변 가능
        param.llm_selector = "openai"
        param.reranker_type = "bge"             # BGE-Reranker로 가짜 유사 문서 사전 차단
        param.do_retriever = "hybrid" 
        model = OpenAIModel(param)
        model.make_model()
        return model    
    elif model_item == "ollama":
        ...
        param.whatmodelItem = "ollama"
        param.embedding_model = "bge-m3"
        param.llm_model = "exaone3.5:7.8b"
        param.temperature = 0.2          # 문장 흐름을 위해 약간의 유연성 부여
        param.repetition_penalty = 1.15  # 반복 방지 및 답변 품질 향상
        param.chunk_size = 1000          # 1200보다 살짝 줄여 컨텍스트 부하 최적화
        param.chunk_overlap = 150        # 충분한 오버랩으로 문맥 단절 방지
        param.k = 13                     # 풍부한 정보량 제공
        param.llm_selector = "ollama"
        param.retriever_llm_type = "ollama"
        param.reranker_type = "bge"             # 필수! 많은 정보 중 핵심만 LLM 상단에 배치
        param.do_retriever = "hybrid"     # 키워드(BM25)와 의미(Vector)를 모두 잡음
        model = OllamaModel(param)
        model.make_model()
        return model
